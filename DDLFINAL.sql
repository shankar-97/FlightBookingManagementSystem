...
--VIEW TO DISPLAY PASSENGERS VACCINATED FROM INDIA
CREATE VIEW V_PASS_VACC_IND 
AS
...
--VIEW TO DISPLAY FIRSTNAME, LAST NAME, COUNTRY where TRANSACTION STATUS COMPLETED AND PASSENGER IS A ADULT
...
--TABLEAU DATA
--DISPLAYING PASSENGER NAME,AGE,country, status, passengertype, IMMUNIZATION,origin, dest

SELECT Age,TrasactionStatus,Passengertype,OriginAirportID,DestinationAirportID,
ImmunizationName,ImmunizationCountry FROM PERSON P 
JOIN BOOKING B 
ON P.PersonID=B.PersonID
JOIN PASSENGER PA 
ON B.PassengerID=PA.PassengerID
...
ON T.FlightScheduleID= FS.FlightScheduleID
JOIN [ROUTE] R
ON FS.RouteNumber= R.RouteNumber
JOIN IMMUNIZATION I
ON PA.PassengerID=I.PassengerID
ORDER BY 1

-- DATA

AIRLINE  NAME
TRIP BAGGAGE ALLOWNCE
ROUTE
FS SeatAvailable
Ticket
Booking
