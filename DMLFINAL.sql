-------------------------- DML QUERIES (INSERTION STATEMENTS, STORED PROCEDURES, TRIGGERS, VIEWS)-----------------------------------------

----------------------------------------------------INSERTION STATEMENTS----------------------------------------------------------------

------------------------------------------------------------PERSON----------------------------------------------------------------------

INSERT INTO PERSON (FirstName,LastName,Birthdate,City,State,Country,EmailID,MobileNumber,PersonType) 
VALUES ('Cristiano','Ronaldo','1985-02-05','Funchal','Madeira','Portugal','ronnie7@gmail.com','8674532467','Users');
INSERT INTO PERSON (FirstName,LastName,Birthdate,City,State,Country,EmailID,MobileNumber,PersonType) 
VALUES ('Linoel','Messi','2018-06-24','Rosario','Santa Fe','Argentia','messi10@gmail.com','6758910349','Passenger');
INSERT INTO PERSON (FirstName,LastName,Birthdate,City,State,Country,EmailID,MobileNumber,PersonType) 
VALUES 
('Neymar','Junior','1992-02-05','Mogi Das Cruzes','Sau Paulo','Brazil','neymarjr10@rediffmail.com','8742982028','Users');
INSERT INTO PERSON (FirstName,LastName,Birthdate,City,State,Country,EmailID,MobileNumber,PersonType)
VALUES ('Bruno','Fernandes','2005-09-08','Maia','Porto','Portugal','bruno18@gmail.com','9763450238','Passenger');
INSERT INTO PERSON (FirstName,LastName,Birthdate,City,State,Country,EmailID,MobileNumber,PersonType) 
VALUES 
('Wayne','Rooney','2009-10-24','Croxteth','Liverpool','United Kingdom','rooney10@gmail.com','7682346190','Passenger');
INSERT INTO PERSON (FirstName,LastName,Birthdate,City,State,Country,EmailID,MobileNumber,PersonType) 
VALUES ('Xavier','Hernandez','1980-01-25','Terrassa','Catalonia','Spain','xavi06@rediffmail.com','876234590','Users');
INSERT INTO PERSON (FirstName,LastName,Birthdate,City,State,Country,EmailID,MobileNumber,PersonType) 
VALUES ('Sergio','Ramos','2008-03-30','Camas','Seville','Spain','ramos04@yahoo.com','6549823402','Passenger');
INSERT INTO PERSON (FirstName,LastName,Birthdate,City,State,Country,EmailID,MobileNumber,PersonType) 
VALUES ('Nemanja','Vidic','2019-10-21','Uzice','Sumadijia','Serbia','nemanja16@gmail.com','9852346780','Passenger');
INSERT INTO PERSON (FirstName,LastName,Birthdate,City,State,Country,EmailID,MobileNumber,PersonType) 
VALUES 
('Marcelo','Veiria','1988-05-12','Rio De Janeiro','Rio De Janeiro','Brazil','marcelo12@gmail.com','7654389012','Passenger');
INSERT INTO PERSON (FirstName,LastName,Birthdate,City,State,Country,EmailID,MobileNumber,PersonType) 
VALUES ('Carlos','Puyol','2010-04-13','Segur','Pallars Jussa','Brazil','carlos05@yahoo.com','9865234802','Users');
INSERT INTO PERSON (FirstName,LastName,Birthdate,City,State,Country,EmailID,MobileNumber,PersonType) 
VALUES ('Iker','Casillas','1981-05-20','Mostoles','Madrid','Spain','iker01@gmail.com','9876589021','Users');
...
--STORED PROCEDURE 1
--Get the count of cabin crew on board based on the airline id
CREATE PROC Totalcountt @AirlineID int, @crewcount int OUTPUT AS

BEGIN

SELECT @crewcount = t.crewmemberscount
FROM trip t join airline a on t.airlineid = a.airlineid
WHERE a.airlineid = @AirlineID
END

DECLARE @crewcount int
EXEC totalcountt 2, @crewcount output
SELECT @crewcount as 'Cabin OnBoard'



--STORED PROCEDURE 2
--Get all routes from "ROUTE" table and seats available for the flights to a Selected Destination Airport from "FLIGHT SCHEDULE" table.
CREATE PROCEDURE AvailableSeats @DestinationAirportID char(3)  AS
BEGIN 

  SELECT r.OriginAirportID, r.DestinationAirportID, f.seatavailable
  FROM FlightSchedule f join [Route] r on f.RouteNumber = r.RouteNumber
  WHERE r.DestinationAirportID = @DestinationAirportID

END

EXEC AvailableSeats 'BOS'


---STORED PROCEDURE 3
-- This procedure takes location_id  of office from "WORK LOCATION" table  as input and returns the city it is located in as output
CREATE PROCEDURE GetCountry @locationid int, @locname varchar(50) output 
AS
BEGIN

SELECT @locname = w.locationname
from Worklocation w
WHERE w.locationid = @locationid
end

DECLARE @locname varchar(50)
EXEC GetCountry 1003, @locname output
SELECT @locname as 'CITY'


----------------------------------------------------------------VIEWS--------------------------------------------------------------------

--VIEW 1
--VIEW TO DISPLAY PASSENGERS VACCINATED FROM SPAIN
CREATE VIEW V_PASS_VACC_SPA 
AS
SELECT P.PassengerID,ImmunizationName 
FROM PASSENGER P
JOIN IMMUNIZATION I
ON P.PassengerID=I.PassengerID 
WHERE ImmunizationCountry='SPAIN';

SELECT * FROM V_PASS_VACC_SPA ;  --To RUN and display the VIEW


-- VIEW 2  TO DISPLAY FIRSTNAME, LAST NAME, COUNTRY where TRANSACTION STATUS COMPLETED AND PASSENGER IS A ADULT
CREATE VIEW V_NAME_STAT_PASS
AS 
...
WHERE TrasactionStatus like 'Comp%'
AND PassengerType= 'ADULT';

SELECT * FROM V_NAME_STAT_PASS   --To RUN and display the VIEW


--------------------------------------------------------------------TRIGGER---------------------------------------------------------------

-- This Trigger is used to maintain a list of all updations happening on the PERSON table	
	--Creating an AUDIT TABLE for "PERSON"
...
CREATE TRIGGER UpdateTrigger on [Person]
FOR UPDATE
AS DECLARE @PersonID int,
...
SELECT @PersonID = ins.PersonID FROM INSERTED ins;
SELECT @FirstName = ins.FirstName FROM INSERTED ins;
SELECT @LastName = ins.LastName FROM INSERTED ins;
SELECT @Birthdate = ins.Birthdate FROM INSERTED ins;
SELECT @City = ins.City FROM INSERTED ins;
SELECT @State = ins.[State] FROM INSERTED ins;
SELECT @Country = ins.Country FROM INSERTED ins;
SELECT @EmailID = ins.EmailID FROM INSERTED ins;
SELECT @MobileNumber = ins.MobileNumber FROM INSERTED ins;

IF UPDATE(FirstName)
BEGIN
     SET @Action = 'FirstName Modified'
END

IF UPDATE(LastName)
BEGIN
     SET @Action = 'LastName Modified'
END

IF UPDATE(Birthdate)
BEGIN
     SET @Action = 'Birthdate Modified'
END
IF UPDATE(City)
BEGIN
     SET @Action = 'City Modified'
END

IF UPDATE([State])
BEGIN
     SET @Action = 'State Modified'
END

IF UPDATE(Country)
BEGIN
     SET @Action = 'Country Modified'
END

IF UPDATE(EmailID)
BEGIN
     SET @Action = 'EmailID Modified'
END

IF UPDATE(MobileNumber)
BEGIN
     SET @Action = 'MobileNumber Modified'
END

INSERT INTO [PersonAudit] VALUES(
	--@PersonID,
	@FirstName,
	@LastName,
	@Birthdate,
...
--Running the below code displays Two results with Updation message and Modified table
